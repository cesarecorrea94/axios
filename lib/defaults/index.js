...
const defaults = {
   ...
   * - `'lookup'`: Emitted by the `socket` after resolving the host name but before connecting.
   ...
   * - `'end'`: Emitted by the `socket` when the other end of the socket signals the end of transmission,
   *          thus ending the readable side of the socket.
   * - `'activity'`: Corresponds to `socket.setTimeout(timeout)` upon the emission of `startShot`,
   *                so the timeout refreshes on each subsequent event until the end of the request.
   *
